...
import Inquiries from "./AdminDashboard/Inquiries";
import NotificationsPage from "./AdminDashboard/NotificationsPage";
import AdminTimeTable from "./AdminDashboard/AdminTimeTable";
import PaymentStatus from "./AdminDashboard/PaymentStatus";
...
const AdminDashboard = () => {
  ...
  const sendSignupLinks = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/auth/send-signupLinks`, {
        emails: emailList,
        message,
      });
          alert("Signup links sent successfully!");
  ...
  const renderContent = () => {
    switch (activeSection) {
      case "home":
        return <Home />;
      case "send-signup-links":
        return (
          <div className="p-6">
            <h1 className="text-2xl font-bold mb-4">Send Signup Links</h1>
            <textarea value={emails} onChange={(e) => setEmails(e.target.value)}
              className="w-full px-3 py-2 border rounded" rows="3" placeholder="Enter emails, comma-separated" />
            <textarea value={message} onChange={(e) => setMessage(e.target.value)}
              className="w-full px-3 py-2 border rounded mt-4" rows="4" placeholder="Write your message here..." />
            <button onClick={sendSignupLinks} className="px-4 py-2 bg-orange text-white rounded mt-4">Send</button>
            <ToastContainer />
          </div>
        );
      ...
      case "consult": return <Inquiries />;
      case "fees" : return <PaymentStatus />
      case "notifications": return <NotificationsPage />;
      case "schedule": return <AdminTimeTable />;
