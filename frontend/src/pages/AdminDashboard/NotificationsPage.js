...
const NotificationsPage = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/notifications/admin`, {
        headers: {
    ...
    <div className="p-6">
      ...
      <form onSubmit={sendNotification} className="bg-gray-100 p-4 rounded-lg mb-6">
        ...
        >
          ...
          <option value="specific_teacher_class">Specific Teacher (Class)</option>
          <option value="specific_teacher_course">Specific Teacher (Course)</option>
          <option value="specific_teacher_subjects">Specific Teacher (Subject)</option>
        </select>

        {(formData.recipientType === "specific_class" ||
          formData.recipientType === "specific_teacher_class") && (
          <input
            type="text"
            name="className"
        ...
        {(formData.recipientType === "specific_course" ||
          formData.recipientType === "specific_teacher_course" )&& (
          <input
            type="text"
            name="courseName"
        ...
        {(formData.recipientType === "specific_subject" ||
          formData.recipientType === "specific_teacher_subjects") && (
  <>
    <input
      type="text"
      name="className"
          ...
          notifications.map((notif) => (
            <div key={notif._id} className="p-4 border rounded flex justify-between items-center bg-white shadow">
              <div>
                ...
                <p>{notif.message}</p>
                <p className="text-sm text-gray-600">By: {notif.sender ? notif.sender.name : 'Unknown Role'}</p>
              </div>
              <button
                onClick={() => deleteNotification(notif._id)}
                className="bg-orange text-white px-3 py-1 rounded"
              >
                                   Delete    <FaTrash className="inline-block mr-2 ml-2 text-xm" />
