...
const StudentPayment = () => {
  ...
  const fetchFeeDetails = async () => {
    try {
      ...
      if (!amount || amount === 0) {
        setAmount(response.data.fullYearlyFee); // ✅ Set full-year fee if no previous payment
      }
    } catch (error) {
  ...
  const handleInstallmentChange = (plan) => {
    ...
    let totalFee = feeDetails?.fullYearlyFee || amount;
    let paidPayments = userPayments.filter(payment => payment.status !== "pending");
    let remainingAmountCalc = totalFee - paidPayments.reduce((acc, payment) => acc + payment.paidAmount, 0);

    let installmentAmountCalc = remainingAmountCalc / (plan === "3 Months" ? 3 : 6);
  ...
  const handlePayment = async () => {
    ...
    const storedUser = storedData.user;
    const orderAmount = userPayments.length > 0 ? userPayments[0].amount : amount;
    ...
    try {
      const response = await axios.post(
        ...
        { amount: orderAmount, paymentType, installmentPlan },
        { headers: authHeader() }
      );
